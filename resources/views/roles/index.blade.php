            ...
            <div class="hbox hbox-auto-xs hbox-auto-sm" ng-init="
                ...
                <div class="col">
                    ...
                    <div class="bg-light lter b-b wrapper-md">
                        <div class="row mb-2">
                            <div class="col-sm-6">
                                <h1 class="m-0 text-uppercase">Danh sách quyền</h1>
                            </div>
                            <div class="col-sm-6" style="float: right; text-align: right;">
                                <a href="{{ route('roles.create') }}" class="btn m-b-sm btn-sm btn-primary btnAddRoles pull-right m-l btn-addon">
                                                ...
                                                <td>
                                                    <label class="i-switch bg-info m-t-xs m-r">
                                                        @if(intval($item->status) == 0)
                                                            <input class="publicRoles" id="{{ _encode($item->id) }}" type="checkbox" checked />
                                                            <i></i>
                                                        @else
                                                            <input class="publicRoles" id="{{ _encode($item->id) }}" type="checkbox" />
                                                            <i></i>
                                                        @endif
                                                    </label>
                                                </td>
                                                <td>
                                                    <a href="{{ route('roles.edit', ['id' => _encode($item->id)]) }}" class="text-primary m-t-xs update">
                                                        <i class="glyphicon glyphicon-pencil"></i> Sửa
                                                    </a>
                                                    <a href="{{ route('roles.delete', ['id' => _encode($item->id)]) }}" class="text-danger m-t-xs delete ms-3">
